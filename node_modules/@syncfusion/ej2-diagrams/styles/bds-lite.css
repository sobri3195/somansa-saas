...
.e-diagram-resize-handle.e-disabled {
  fill: var(--color-sf-bg-secondary-alt);
  opacity: 1;
  stroke: var(--color-sf-base-white);
}
...
.e-symbolpalette .e-acrdn-content {
  background-color: var(--color-sf-base-white) !important;
}
...
.e-diagram-selector.e-disabled {
  opacity: 1;
  stroke: var(--color-sf-bg-secondary-alt);
}
...
.e-ruler-tick-label {
  fill: var(--color-sf-text-tertiary);
}
