    ...
    public function index()
    {
        if (Auth::user()->can('manage-offers')) {
            ...
            return Inertia::render('Recruitment/Offers/Index', [
                ...
                'jobpostings' => JobPosting::where('created_by', creatorId())->select('id', 'title')->get()->map(function ($job) {
                    return [
                        'id' => $job->id,
                        'name' => $job->title
                    ];
                ...
                'departments' => \Workdo\Hrm\Models\Department::where('created_by', creatorId())->select('id', 'department_name as name')->get(),
            ]);
        } else {
