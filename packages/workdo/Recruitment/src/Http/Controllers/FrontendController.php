...
class FrontendController extends Controller
{
    ...
    public function jobListings(Request $request, $userSlug)
    {
        try {
            ...
            // Get recruitment settings for this user
    ...
    public function applicationSuccess(Request $request, $userSlug, $trackingId = null)
    {
        try {
            ...
            // Get candidate data by tracking ID
            $candidate = null;
            $errorMessage = null;
            ...
            if ($trackingId) {
                ...
                // If tracking ID provided but candidate not found, redirect with clean URL
                if (!$candidate) {
                    // Extract valid tracking ID pattern (remove extra characters)
                    ...
                    $errorMessage = 'Application not found. Please check your tracking ID.';
                }
            }
        ...
        try {
            ...
            if (!$candidate) {
                return back()->withErrors([
                    'message' => 'No application found with the provided tracking ID and email address. Please check your details and try again.'
                ]);
            }
    ...
    public function trackingDetails(Request $request, $userSlug, $trackingId)
    {
        try {
            ...
            // Find candidate and load job details with interviews and offers
            $candidate = Candidate::where('tracking_id', $trackingId)
                ->where('created_by', $userId)
                ->with([
                    'job_posting.jobType',
    ...
    private function generateTimeline($candidate, $currentStatus, $interviewDetails = null, $offerDetails = null)
    {
        ...
        if ($currentStatus >= 1) {
            $timeline[] = [
                'status' => 1,
                'title' => 'Screening',
                'description' => 'Your application is being screened by our team',
        ...
        if ($currentStatus >= 2) {
            ...
            if ($interviewDetails) {
                $description = 'Interview scheduled for ' . $interviewDetails['date'] . ' at ' . $interviewDetails['time'];
        ...
        if ($currentStatus >= 3) {
            ...
            if ($offerDetails) {
                $description = 'Job offer extended - expires on ' . $offerDetails['expiration_date'];
        ...
        if ($currentStatus == 5) {
            ...
            $timeline[] = [
                ...
                'description' => 'We will notify you of our final decision',
                'date' => null,
                'completed' => false
            ];
        }
    ...
    public function offerResponse(Request $request, $userSlug, $offerId)
    {
        try {
            ...
            if (!$offer) {
                return back()->withErrors(['message' => 'Offer not found']);
            }
            ...
            $message = $request->status === '2' ? 'Offer accepted successfully!' : 'Offer declined successfully.';
        ...
        try {
            ...
            if (!$candidateSource) {
                $candidateSource = CandidateSources::create([
                    'name' => 'Career Portal',
                    'description' => 'Applications from career portal',
                    'is_active' => true,
                    'created_by' => $userId,
                    'creator_id' => $userId
                ]);
            ...
            return response()->json(['error' => 'An error occurred while submitting your application. Please try again.'], 500);
        }
    }
}
