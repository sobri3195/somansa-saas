...
class DemoOfferSeeder extends Seeder
{
    public function run($userId): void
    {
        ...
        // Get all candidates for demo purposes
        $eligibleCandidates = Candidate::where('created_by', $userId)->get();
        ...
        for ($i = 0; $i < 15; $i++) {
            ...
            $job = JobPosting::where('created_by', $userId)->inRandomOrder()->first();
            $approver = User::where('created_by', $userId)->inRandomOrder()->first();
            ...
            if ($status === 2 && $approvalStatus === 'approved') { // Accepted and approved
                ...
                if ($convertedToEmployee) {
                    $employee = Employee::where('created_by', $userId)->inRandomOrder()->first();
